...
import {OurFields} from '../ScrollNav';
    ...
    >
      ...
      {open ?
        <Box boxShadow={3} borderRadius={1} display='flex' width='350px' backgroundColor='white' position='absolute' flexDirection='column' onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} paddingY={1} sx={{maxHeight: '300px', overflowY: 'auto', zIndex: 6 }}>
          {OurFields?.length > 0 ? (
            OurFields.map((facility) => (<Box key={facility.id} padding={1} onClick={() => setOpen(false)} sx={{ cursor: 'pointer', '&:hover': { backgroundColor: 'lightgray', display: 'flex', width: '100%' } }}>
              <Link href={`/whatwedo`} passHref legacyBehavior>
